...
void dir_join(char *dst, const char *dirname, const char *filename, unsigned int length) {
    strncpy(dst, dirname, length - 2);
    if (dst[strlen(dst) - 1] != '/')
        strcat(dst, "/");
...
void url_decode(char *dst, const char *src, unsigned int length)
{
    unsigned int l = 0;
    for (;;)
    {
